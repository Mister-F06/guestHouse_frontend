<template>
  <div>
        ...
        <div class="table-container m-5">
          <table class="table is-bordered is-hoverable is-fullwidth">
            <tr>
              ...
              <th style="text-align: right" v-if="role_id && role_id === 2">
                Visibilité
              </th>
            ...
            <tr v-for="item in displayedGuesthouse" :key="item.id">
              ...
              <td style="text-align: left">
                {{ formatNumberCustom(item.price) }} <span>FCFA</span>
              </td>
              <td style="text-align: left">{{ item.address }}</td>
              <td style="text-align: right" v-if="role_id && role_id === 1">
                <span v-if="item.status == 'validated'" class="tag is-primary is-light"
                  >Valider</span
                >
                <span
                ...
                >
                <span v-if="item.status == 'rejected'" class="tag is-danger is-light"
                  >Rejeter</span
                >
                <a v-b-modal.modal-5 variant="primary" @click="openStatusModal(item)">
                  <i
                    class="fa fa-edit is-small"
                    style="font-size: small; color: green"
                  ></i>
                </a>
              </td>
              <td style="text-align: right" v-if="role_id && role_id === 2">
                <span v-if="item.is_enabled == true" class="tag is-primary is-light"
                  >Visible</span
                >
                <span v-if="item.is_enabled == false" class="tag is-danger is-light"
                  >Invisible</span
                >
              ...
              <td style="text-align: right">
                <a
                  ...
                  v-b-modal.modal-3
                  variant="danger"
                  @click="giveId(item.id)"
                >
                  <span class="icon is-small">
                    <i
                      class="fa fa-trash is-small"
    ...
    <b-modal
      id="modal-1"
      :title="updated_ == false ? 'Ajouter un gueshouse' : 'Modifier le gueshouse'"
      size="lg"
      hide-footer
    >
      <validation-observer v-slot="{ handleSubmit }" ref="formValidator">
        <b-form
          role="form"
          @submit.prevent="
            handleSubmit(updated_ == false ? addGuesthouse : updateGuesthouse)
          "
        >
          <b-row>
            <b-col>
          ...
          <b-row :class="updated_ ? 'visibilityClass' : 'texting'">
            <b-col>
              <upload
                ...
                :file-list="fileList"
              >
                <base-button size="sm" type="primary" class="my-4"
                  >Cliquer pour envoyer le Cover</base-button
                >
            ...
            <b-col>
              ...
              >
                <base-button size="sm" type="primary" class="my-4"
                  >Cliquer pour envoyer les images</base-button
                >
                <div slot="tip" class="el-upload__tip">
                  Fichiers jpg/png avec une taille inférieure à 500kb
                </div>
            ...
            <b-col>
              ...
              >
                <base-button size="sm" type="primary" class="my-4"
                  >Cliquer pour envoyer les vidéos</base-button
                >
                <div slot="tip" class="el-upload__tip">
                  Fichiers mp4/webp avec une taille inférieure à 500kb
                </div>
              </upload>
            </b-col>
          </b-row>
    ...
    <b-modal id="modal-3" title="Confirmation de suppression" size="sm" hide-footer>
      ...
      >
        ...
        <img
          src="../assets/undraw_throw_away_re_x60k.svg"
          alt=""
          width="200px"
          class="mb-2"
        />
...
export default {
  ...
  methods: {
    ...
    detailInfoAdd(item) {
      ...
      this.detailInfo.pictures = item.pictures.map((picture) =>
        this.showMediaFromGoogle(picture.original_url)
      );
      this.detailInfo.videos = item.videos.map((video) =>
